...
langDistrib <- languages %>% select(Lang) %>% filter(Lang %in% major_lang) %>% ggplot(aes(x = Lang, y = 100*(..count..)/cleannbp, fill = Lang)) + 
  geom_bar(width = 1, stat = "count", show.legend = FALSE) +
  labs(x = "Language", y = "% Projects") + 
  coord_flip()
langDistrib

ggsave(filename = "langDistrib.png", device="png", height = 4, width = 6,  units = c("in"))
```
